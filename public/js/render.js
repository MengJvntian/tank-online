window.TankRenderer = (() => {
  ...
  function drawTank(ctx,t){const sz=t.size||30,c=sz/2,scale=sz/30;ctx.save();ctx.translate(t.x+c,t.y+c);ctx.rotate({up:0,right:Math.PI/2,down:Math.PI,left:-Math.PI/2}[t.dir]||0);ctx.scale(scale,scale);ctx.fillStyle=t.color||'#22c55e';ctx.fillRect(-15,-15,30,30);ctx.fillStyle='#111827';ctx.fillRect(-12,-12,6,24);ctx.fillRect(6,-12,6,24);ctx.fillStyle='#e5e7eb';ctx.fillRect(-4,-25,8,24);ctx.fillStyle=t.owner==='enemy'?'#fee2e2':'#fde68a';ctx.beginPath();ctx.arc(0,0,7,0,Math.PI*2);ctx.fill();if(['heavy','shield','elite'].includes(t.kind)||String(t.kind).startsWith('boss')){ctx.strokeStyle=String(t.kind).startsWith('boss')?'#fecdd3':(t.kind==='shield'?'#a5f3fc':t.kind==='elite'?'#bef264':'#f5d0fe');ctx.lineWidth=3;ctx.strokeRect(-12,-12,24,24)}if(t.kind==='sniper'){ctx.fillStyle='#cffafe';ctx.fillRect(-2,-34,4,12)}if(t.kind==='suicide'){ctx.strokeStyle='#fed7aa';ctx.lineWidth=3;ctx.beginPath();ctx.arc(0,0,11,0,Math.PI*2);ctx.stroke()}if(String(t.kind).startsWith('boss')){ctx.fillStyle='#fff';ctx.font='bold 13px Arial';ctx.textAlign='center';ctx.fillText('BOSS',0,4)}ctx.restore();if(t.maxHp>1){ctx.fillStyle='#111827';ctx.fillRect(t.x,t.y-7,sz,4);ctx.fillStyle='#f43f5e';ctx.fillRect(t.x,t.y-7,sz*t.hp/t.maxHp,4)}if(t.shield){ctx.strokeStyle='rgba(96,165,250,.8)';ctx.lineWidth=3;ctx.beginPath();ctx.arc(t.x+c,t.y+c,Math.max(20,sz*.8)+Math.sin(Date.now()/90)*2,0,Math.PI*2);ctx.stroke()}if(t.nick){ctx.textAlign='center';ctx.font='12px Arial';ctx.fillStyle='#e5e7eb';ctx.fillText(t.nick,t.x+c,t.y-12)}}
